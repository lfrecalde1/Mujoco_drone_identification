...
f_obj1 = @(x) funcion_costo_theta_d(x, N, u_ref, u, up, theta_ref);
...
for k=1:length(t)
    theta_est(k) =  x(1)*(ul_ref(1, k)) - x(2)*(ul(1, k))  +x(3)*ul_p(1, k); 
end
...
subplot(1,1,1)
plot(t(1:length(theta_ref)),theta_ref(1,1:length(t)),'-','Color',[226,76,44]/255,'linewidth',1); hold on
grid on;
plot(t(1:length(theta_est)),theta_est(1,1:length(t)),'--','Color',[100,50,40]/255,'linewidth',1); hold on
