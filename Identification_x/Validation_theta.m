...
chi_theta = [0.6003    0.0010    0.6007];
%% Estimation Optimization Problem
for k=1:length(t)
    ty_est(k) = chi_pid(1)*(theta_ref(k)) - chi_pid(2)*(theta(k))  + chi_pid(3)*theta_p(k) + chi_pid(4)*theta_pp(k); 
    theta_pp_estimado(k) =  (-chi_theta(1)/chi_theta(2))*(1/(1-chi_pid(4)/chi_theta(2)))*phi_p(k)*psi_p(k) - (chi_theta(2)/chi_theta(2))*(1/(1-chi_pid(4)/chi_theta(2)))*phi_p(k)*psi_p(k) + (chi_theta(3)/chi_theta(2))*(1/(1-chi_pid(4)/chi_theta(2)))*phi_p(k)*psi_p(k) + (1/chi_theta(2))*(chi_pid(1)*(theta_ref(k)) - chi_pid(2)*(theta(k))  + chi_pid(3)*theta_p(k));
...
grid on;
plot(t(1:length(ty_est)),theta_pp_estimado(1,1:length(t)),'--','Color',[100,50,40]/255,'linewidth',1); hold on
